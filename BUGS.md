...
|---|---|---|---|
| B-78 | High | `getTestimonials` is wrapped in `cachedRead`, and Next's data cache stores what it is handed as **JSON** — so the `Date` in `stayDate` came back a string on every cache hit while `SiteTestimonial` still declared it a `Date`. The home page believed the type and called `x.stayDate.toLocaleDateString(...)`, so `/` died with `TypeError: stayDate.toLocaleDateString is not a function` — at prerender it **failed `next build`**, and a deployed build would have 500'd the home page on the first cache hit after each 60s window. Latent until B-74's follow-up moved `/` from `force-dynamic` to ISR. Converted below the cache so `cachedRead`'s two paths (through the cache in a page, straight through in `perf-queries.ts` and the `verify-*.ts` scripts) cannot return different types for the same field | `lib/site-content.ts`, `app/[locale]/page.tsx`, `__tests__/unit/lib/site-content.test.ts` |
| B-77 | Low | `WalkInModal.test.tsx > "omits nightlyRate entirely when left blank"` intermittently failed under the full parallel run with `Unable to find role="option" and name /Deluxe Room/`. **Not a missing wait** — the helper already had one. `waitFor` reports its last query failure on timeout, and testing-library's 1s default window is too tight for a machine running 85 files in parallel forks: one file alone took 48s here, 15s of it environment setup. The window is now 5s (`asyncUtilTimeout`, set once in the setup file) with vitest's own timeout raised above it so it cannot mask the cause. 4 consecutive clean full runs since | `__tests__/setup.ts`, `vitest.config.ts`, `__tests__/unit/components/WalkInModal.test.tsx` |
| B-73 | Medium | With `NEXT_PUBLIC_WHATSAPP_NUMBER` unset, the floating button and the UPI confirmation screen both linked to `919876543210` — a real number belonging to someone else, handed a guest's booking reference. All three call sites now go through `whatsappUrl()`, which returns null and renders no button | `lib/whatsapp.ts`, `components/layout/WhatsAppButton.tsx`, `app/[locale]/booking/confirmation/page.tsx`, `app/[locale]/booking/error.tsx`, `__tests__/unit/lib/whatsapp.test.ts` |
| B-75 | Low | `admin-metadata.test.ts` shelled out to `grep` with `execSync` from inside a vitest worker; under a full parallel run on Windows the spawn failed intermittently, reddening an otherwise green suite at random. Replaced with Node's own `fs` | `__tests__/unit/lib/admin-metadata.test.ts` |
| B-76 | High | `/api/payment/verify` was the only path to `paid`, and it runs in the guest's browser. A closed tab, a dropped connection or one 500 from that route left Razorpay holding the money and the booking at `pending` with no `Payment` row — so the stay was absent from every revenue report, reconciliation total and occupancy figure until a human read a log line. `expireStalePaymentHolds()` covered only half of it: it asks Razorpay, is told `"paid"`, and keeps the room — but nothing confirmed the booking or emailed the guest, who had paid and been told nothing | `app/api/payment/webhook/route.ts`, `lib/payment-settlement.ts`, `lib/razorpay.ts`, migration `13_payment_idempotency`, `app/api/payment/verify/route.ts` |
| B-74 | High | The home page was **statically prerendered** — the only database-reading public page with no `force-dynamic`, so `next build` wrote `.next/server/app/en.html` with the content baked in. Approving a testimonial or repricing a room changed nothing at `/` until the next deploy: `Priya Sharma` and `priceRange":"₹4500-₹7500"` were literals in that file, and the frozen price went out in the schema.org graph too. Found by perf testing — `/` answered in 17ms while `/gallery` and `/blog`, doing the same kind of read, took ~320ms | `app/[locale]/page.tsx` |
